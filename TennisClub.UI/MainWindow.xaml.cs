...
namespace TennisClub.UI
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void AddMemberFineButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (int.TryParse(MemberFineFineNumber.Text, out var fineNumber) &&
                decimal.TryParse(MemberFineAmount.Text, out var amount) && !member.IsNull() && !handoutDate.IsNull())
            {
        ...
        private void SynchroniseRoleTable()
        {
            ...
            if (isSucceeded)
            {
                MessageBox.Show("De tabel is succesvol gesynchroniseerd met de database!", "Succes!",
        ...
        private void GameResultGameId_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            gameResult.GameId = int.Parse(GameResultGameId.SelectedValue.IsNull()
                ? "0"
                : GameResultGameId.SelectedValue.ToString());
            GameResultData.Items.Refresh();
        }
        ...
        private void AddMemberRoleButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!startDate.IsNull() && !member.IsNull() && !role.IsNull())
            {
                ...
                if (newList.Exists(x =>
                    x.MemberId == member.Id && x.RoleId == role.Id && x.StartDate.Equals(startDate) &&
                    x.EndDate.Equals(endDate)))
                {
                    MessageBox.Show("Er is al een rol toegewijzen aan de gekozen speler op de ingevoerde data!",
                        "Fout!", MessageBoxButton.OK,
                        MessageBoxImage.Error);
                    return;
                }
